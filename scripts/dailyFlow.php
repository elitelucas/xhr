...
function index($start_time,$end_time)
{
    ...
    if(empty($userList)){
        @file_put_contents('dailyFlow.log', " NULL-1: ". $sql .  "\n", FILE_APPEND);
        return true;
    }
    ...
    foreach ($userList as $v) {
        ...
        if(empty($tradeLog)){
            @file_put_contents('dailyFlow.log'," NULL-2: ". $sql .  "\n", FILE_APPEND);
            continue;
        }
        ...
        foreach ($tradeLog as $vt) {
            ...
            $res = $db->query($sql);
            if(!$res){
                @file_put_contents('dailyFlow.log', " SQL: ".$sql .  "\n", FILE_APPEND);
            }
            @file_put_contents('dailyFlow.log', "用户id :" . $v['user_id'] . " |  类型: ".$vt['type']." | " . $res ." | " .$addtime. "\n", FILE_APPEND);
        }
    }
}
