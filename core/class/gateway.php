...
class Gateway
{
    ...
    public function sendToClient2($client_id, $message)
    {
        return self::sendCmdAndMessageToClient($client_id, GatewayProtocol::CMD_SEND_TO_ONE, $message);
    }
    /**
    ...
    public static function getAllClientSessions($group = null)
    {
        ...
        $status_data      = array();
//        lg('total','$gateway_data-------------->'.json_encode($gateway_data,JSON_UNESCAPED_UNICODE));
        $all_buffer_array = self::getBufferFromAllGateway($gateway_data);
//        lg('total','$all_buffer_array-------------->'.json_encode($all_buffer_array,JSON_UNESCAPED_UNICODE));
    ...
    /**
     * 获取所有真人在线人数  add by joinsen
     *
     * 2018-09-05 update
     * 改成统计uid
     *
     * @param string $group
     * @return $total_count 真人数量
     */
    public static function getAllClientRealMan($group = null)
    {
        $gateway_data = GatewayProtocol::$empty;
        $gateway_data['cmd'] = GatewayProtocol::CMD_GET_ALL_CLIENT_INFO;
        $status_data      = array();
            ...
            foreach ($buffer_array as $local_port => $data) {
                if ($data) {
                    foreach ($data as $connection_id => $session_buffer) {
                        ...
                        if(array_key_exists('reg_type',$status_data[$client_id]) && $status_data[$client_id]['roomid']>0 && !in_array($status_data[$client_id]['reg_type'],array(0,8,9,11))){ //判断在房间，并且是真人
                            if(!in_array($status_data[$client_id]['userid'],$uidsArr)) {
                                $uidsArr[] = $status_data[$client_id]['userid'];
                                $total_count++;
                            }
                        }
                    }
     ...
     * @return $total_count 真人数量
     */
    public static function getAllUidByRoom($group = null)
    {
        $gateway_data = GatewayProtocol::$empty;
        $gateway_data['cmd'] = GatewayProtocol::CMD_GET_ALL_CLIENT_INFO;
        $status_data      = array();
            ...
            foreach ($buffer_array as $local_port => $data) {
                if ($data) {
                    foreach ($data as $connection_id => $session_buffer) {
                        ...
                        if(array_key_exists('reg_type',$status_data[$client_id]) && $status_data[$client_id]['roomid']>0 && !in_array($status_data[$client_id]['reg_type'],array(0,8,9,11))){ //判断在房间，并且是真人
//                            lg('get_room_online',var_export(array(
//                                '$total_count'=>$total_count,
//                                '$status_data'=>$status_data,
////                                '$uidsArr[$status_data[$client_id][\'roomid\']]'=>$uidsArr[$status_data[$client_id]['roomid']],
////                                '(!in_array($status_data[$client_id][\'userid\'],$uidsArr[$status_data[$client_id][\'roomid\']]))'=>(!in_array($status_data[$client_id]['userid'],$uidsArr[$status_data[$client_id]['roomid']])),
//                            ),1));
                            if(!in_array($status_data[$client_id]['userid'],isset($uidsArr[$status_data[$client_id]['roomid']])?$uidsArr[$status_data[$client_id]['roomid']]:[])) {
                                $uidsArr[$status_data[$client_id]['roomid']][] = $status_data[$client_id]['userid'];
                                $total_count[$status_data[$client_id]['roomid']]++;
                            }
                        }
                    }
                }
     ...
     * @return $total_count 真人数量
     */
    public static function getAllUidByRoomID($group = null)
    {
        $gateway_data = GatewayProtocol::$empty;
        $gateway_data['cmd'] = GatewayProtocol::CMD_GET_ALL_CLIENT_INFO;
        $status_data      = array();
...
////                                '(!in_array($status_data[$client_id][\'userid\'],$uidsArr[$status_data[$client_id][\'roomid\']]))'=>(!in_array($status_data[$client_id]['userid'],$uidsArr[$status_data[$client_id]['roomid']])),
//                            ),1));
                            if(!in_array($status_data[$client_id]['userid'],(isset($uidsArr[$status_data[$client_id]['roomid']])?$uidsArr[$status_data[$client_id]['roomid']]:[])) && $status_data[$client_id]['roomid'] == $group) {
                                $uidsArr[] = $status_data[$client_id]['userid'];
                            }
                        }
                    }
