 ...
 * Created by Kevin.
 * @author kevin
 * @copyright HCHT 2017/12/29 10:30
 * @description 多得宝支付
 */

include_cache(S_CORE . 'class' . DS . 'pay' . DS . 'payinfo' . '.php');

class DuoDeBaoPay extends PayInfo
{
    //请求接口Url
    public $url      = 'https://api.ddbill.com/gateway/api/scanpay';  //扫码正式（线上）调用接口
    public $bank_url = 'https://pay.ddbill.com/gateway?input_charset=UTF-8';  //第三方网银接口
    public $payName  = '多得宝支付';     //支付类型名称
    //获取支付返回数据格式
    ...
    public $orderInfo = [            //异步验签结果返回格式
        'code'        => 1,                 //0：数据获取成功，其他数字，数据获取失败
        'bank_num'    => 231050,        //银行区分号（不同支付的前三位不同）
        'order_no'    => '',            //后台数据库支付订单号
        'amount'      => 0,               //支付金额
        'ret_error'   => 0,            //回调处理失败时，返回接口字符串
        'ret_success' => 'SUCCESS',  //回调处理成功时，返回接口字符串
        'bank_name'   => '多得宝支付',       //支付方式名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            ...
            'service_type'  => $config['payType'][$data['pay_type']]['payStr'],
        ...
        if ($data['pay_type'] == 'wy') {
            ...
            $post_data['pay_type'] = 'b2c,plateform,dcard,express,weixin,alipay';
            $post_data['input_charset'] = 'UTF-8';
            $post_data['client_ip_check'] = 0;
            $post_data['redo_flag'] = 1;
            $post_data['interface_version']  = 'V3.0';   //接口版本，固定值：V3.0(必须大写)
            ...
            //非网银支付，微信、支付宝、QQ钱包支付
            //签名数据连接成字符串
            $post_data['interface_version'] = 'V3.1';       //接口版本，固定值：V3.0(必须大写)
            $signStr = $this->getUrlParams($post_data, ['sign_type',]);
            //初始化商户私钥
            $merchant_private_key = openssl_get_privatekey($config['merchantPrivateKey']);
            //RSA-S签名
            ...
            if (!isset($curlData['data']['result_code']) || $curlData['data']['result_code'] != 0 || !isset($curlData['data']['resp_code']) || $curlData['data']['resp_code'] != 'SUCCESS') {
