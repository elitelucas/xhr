 ...
 * Created by Kevin.
 * @author kevin
 * @copyright HCHT 2017/12/26 9:30
 * @description 乐盈支付
 */

include_cache(S_CORE . 'class' . DS . 'pay' . DS . 'payinfo' . '.php');

class LeYingPay extends PayInfo
{
    //请求接口Url
    //public $url = 'https://www.funpay.com/website/pay.htm';               //扫码测试调用接口
    public $url = 'https://www.funpay.com/website/pay.htm';               //扫码正式（线上）调用接口
    public $bank_url = 'https://www.funpay.com/website/pay.htm';  //第三方网银接口
    public $payName = '乐盈支付';    //接口名称
    ...
    public $orderInfo = [            //异步验签结果返回格式
        'code' => 1,                 //0：数据获取成功，其他数字，数据获取失败
        'bank_num' => 229050,        //银行区分号（不同支付的前三位不同）
        'order_no' => '',            //后台数据库支付订单号
        'amount' => 0,               //支付金额
        'ret_error' => 0,            //回调处理失败时，返回接口字符串
        'ret_success' => 'SUCCESS',  //回调处理成功时，返回接口字符串
        'bank_name' => '乐盈支付',   //支付方式名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            ...
            'couponFlag'      => 0,
            'platformID'      => '',
            'returnUrl'       => "https://".$_SERVER['HTTP_HOST']."/?m=web&c=pay&a=payOk&order_sn=" . $orderInfo,
            'noticeUrl'       => "https://".$_SERVER['HTTP_HOST']."/?m=api&c=pay&a=doPaycallBack&payment_id=" . $data['payment_id'],
