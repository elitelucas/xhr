...
class XinSunMa extends PayInfo
{
    //请求接口Url

    public $url = 'http://gateway.qiantopay.com/online/gateway';               //扫码正式（线上）调用接口
    public $h5_url = 'http://gateway.qiantopay.com/online/gateway';
    public $payName = '新孙码支付';   //接口名称
...
//            'callbackurl'          => "https://".$callbackurl."/rechargeNotify.php",
        ...
        $post_data['hrefbackurl'] = "https://".$callbackurl."/?m=web&c=recharge&a=rechargeOk&order_sn=" . $orderInfo;

        payLog('xinsunma.txt',print_r($post_data,true));


        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {

            $curlData = $this->httpPost($post_data, $this->url);
            payLog('xinsunma.txt',print_r($curlData,true).'---84--');
            //接口调用成功与否
            if ($curlData['code'] != 200) {
                $this->retArr['code'] = 275073;
                $this->retArr['msg']  = '支付调用失败！';
                payLog('xinsunma.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

                return $this->retArr;
            }
            $returnData = $curlData['data'];
            payLog('xinsunma.txt', print_r($returnData, true).'----113---');
            if (!empty($returnData['result_code'])) {
                $this->retArr['code'] = 275074;
                $this->retArr['msg']  = '支付接口调用失败';
                payLog('xinsunma.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));
...
//            payLog('xinsunma.txt',$url. "   ====137");
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('xinsunma.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('xinsunma.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 275020;
            payLog('xinsunma.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['orderstatus'] != 1) {
            ...
            payLog('xinsunma.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
        ...
        payLog('xinsunma.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 275022;
            payLog('xinsunma.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
...
//        payLog('xinsunma.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
    ...
    public function httpPost($data, $url)
    {
        ...
        payLog('xinsunma.txt',print_r($response,true). "+++251".$url);
//        $response = json_decode(json_encode(simplexml_load_string($response)),true);
    ...
    public function getSign($data, $key)
    {
        $sign_str = '';
        if ($data['sign']) {
            $sign_str = sprintf("partner=%s&ordernumber=%s&orderstatus=%s&paymoney=%s",
                $data['partner'],$data['ordernumber'],$data['orderstatus'],$data['paymoney']);
        } else {
            $sign_str = sprintf("version=%s&method=%s&partner=%s&banktype=%s&paymoney=%s&ordernumber=%s&callbackurl=%s",
                $data['version'],$data['method'],$data['partner'],$data['banktype'],$data['paymoney'],$data['ordernumber'],$data['callbackurl']);
        }
        $sign_str .= $key;
        payLog('xinsunma.txt',$sign_str. "+++++++271");
