...
class yingliPay extends PayInfo
{
    //请求接口Url

    public $url = 'https://www.yinglipay.com/Pay_Index.html';               //扫码正式（线上）调用接口
    public $h5_url = 'https://www.yinglipay.com/Pay_Index.html';
    public $payName = '盈利支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            ...
            'pay_bankcode'      => '0',
//            'pay_attach'      => $data['payment_id'],
//            'pay_notifyurl'          => "https://".$callbackurl."/rechargeNotify.php",
            'pay_notifyurl'          => "https://".$callbackurl."/?m=api&c=recharge&a=rechargeNotify&payment_id=" . $data['payment_id'],
            'pay_callbackurl'      => "https://".$callbackurl."/?m=web&c=recharge&a=rechargeOk&order_sn=" . $orderInfo,
        );

        $post_data['pay_md5sign'] = $this->getSign($post_data, $config['merchantKey']);
        $post_data['cashier'] = $payStr;
        $post_data['tongdao'] = '0';

payLog('yingli.txt',print_r($post_data,true));


        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {
            $curlData = $this->httpPost($post_data, $this->url);
            payLog('yingli.txt',print_r($curlData,true).'---84--');
            
            //接口调用成功与否
            if ($curlData['code'] != 200) {
                $this->retArr['code'] = 275073;
                $this->retArr['msg']  = '支付调用失败！';
                payLog('yingli.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

                return $this->retArr;
            }
            $returnData = $curlData['data'];
            payLog('yingli.txt', print_r($returnData, true).'----113---');
            if ($returnData['returncode'] != '00') {
                $this->retArr['code'] = 275074;
                $this->retArr['msg']  = '支付接口调用失败';
                payLog('yingli.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));

                return $this->retArr;
            }

            //用于安全验证返回url是否非法
            session::set('qrcode_url', $returnData['txcode']);
            session::set('pay_url', '');
            $retData =  [
                'type'     => 1,
                'code_url' => $returnData['txcode'],
                'pay_url'  => '',
                'order_no' => $orderInfo,
...
//            payLog('yingli.txt',$url. "   ====137");
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('yingli.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('yingli.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 275020;
            payLog('yingli.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['returncode'] != '00') {
            ...
            payLog('yingli.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
       ...
       if (isset($data['reserved1'])) {
        ...
        if (isset($data['reserved3'])) {
            unset($data['reserved3']);
        }


        $retSign = $this->getSign($data, $config['merchantKey']);
        payLog('yingli.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 275022;
            payLog('yingli.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
...
//        payLog('yingli.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
    ...
    public function httpPost($data, $url)
    {
        ...
        payLog('yingli.txt',print_r($response,true). "+++251".$url);
...
//        array_filter($data);
        ...
        $sign_str .= "key=".$key;
        payLog('yingli.txt',$sign_str. "+++++++271");
