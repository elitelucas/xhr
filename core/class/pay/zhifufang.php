...
class ZhiFuFang extends PayInfo
{
    //请求接口Url

    public $url = 'http://gateway.zfpfpay.com/simple-web-gateway/scan/apply';               //扫码正式（线上）调用接口
    public $wy_url = 'http://gateway.zfpfpay.com/simple-web-gateway/scan/apply';
    public $payName = '智付方支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            ...
            'tradeType'      => $payStr,
            'tradeTime'      => strval(date("Ymdhis",time())),
            'goodsName'      => 'recharge',
...
//        payLog('zhifuf.txt',print_r($post_data,true));
        $curlData = $this->httpPost($post_data, $this->url);
        payLog('zhifuf.txt',print_r($curlData,true));
        //接口调用成功与否
        if ($curlData['code'] != 200) {
            $this->retArr['code'] = 277073;
            $this->retArr['msg']  = '支付调用失败！';
            payLog('zhifuf.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

            return $this->retArr;
        }
        $returnData = json_decode($curlData['data'],true);
//        payLog('zhifuf.txt', print_r($returnData, true).'----113---');

        if ($returnData['resultCode'] != "0000") {
            $this->retArr['code'] = 277074;
            $this->retArr['msg']  = '支付接口调用失败';
            payLog('zhifuf.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));
...
//            payLog('zhifuf.txt',$url. "   ====137");
            $retData =  [
                'type'  => $config['payType'][$data['pay_type']]['request_type'],
                'modes' => $data['pay_model'],
                'html'  => $this->httpHtml(array(), $returnData['payMsg'])
            ];
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('zhifuf.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('zhifuf.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 277020;
            payLog('zhifuf.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['tradeStatus'] != "0000") {
            ...
            payLog('zhifuf.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
        ...
        payLog('zhifuf.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 277022;
            payLog('zhifuf.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
...
//        payLog('zhifuf.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
...
//        payLog('zhifuf.txt',print_r($response,true). "+++251");
//        $response = json_decode(json_encode(simplexml_load_string($response)),true);
    ...
    public function getSign($data, $key)
    {
        ...
        payLog('zhifuf.txt',$sign_str. "+++++++271");
