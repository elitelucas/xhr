...
class XinBaoPay extends PayInfo
{
    ...
    public $url = 'http://qrpay.naodaida.com/pay/cashier';               //扫码正式（线上）调用接口
    public $h5_url = 'https://api.xhbill.com/gateway/api/h5apipay';
    public $payName = '鑫宝支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            ...
            'service_type'      => $payStr,
            'notify_url'      => "https://".$callbackurl."/rechargeNofify.php",
...
payLog('xinbao.txt', print_r($post_data,true). $payWay);
        $curlData = $this->httpPost($post_data, $url);

        payLog('xinbao.txt',print_r($curlData,true).'---84--');
        //接口调用成功与否
        if ($curlData['code'] != 200) {
            $this->retArr['code'] = 269073;
            $this->retArr['msg']  = '支付调用失败！';
            payLog('xinbao.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

            return $this->retArr;
        }
        $returnData = $curlData['data']['response'];
        payLog('xinbao.txt', print_r($returnData, true).'----113---');

        if (!empty($returnData['result_code'])) {
            $this->retArr['code'] = 269074;
            $this->retArr['msg']  = '支付接口调用失败';
            payLog('xinbao.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('xinbao.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('xinbao.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 274020;
            payLog('xinbao.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));

            return $this->orderInfo;
        }

        //防止错传
        $data['app_id'] = $config['merchantID'];
        $sign = $data['sign'];
        unset($data['sign']);

        $retSign = $this->getSign($data, $config['merchantKey']);
        payLog('xinbao.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 274022;
            payLog('xinbao.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));

            return $this->orderInfo;
        }

        $this->orderInfo['code']      = 0;
        $this->orderInfo['order_no']  = $data['order_sn'];  //商户订单号
        $this->orderInfo['amount']    = $data['amount'] /100;
        $this->orderInfo['serial_no'] = $data['payment_sn'];  //平台订单号
...
//        payLog('xinbao.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
    ...
    public function httpPost($data, $url)
    {
        ...
        payLog('xinbao.txt',print_r($response,true). "+++251");
    ...
    public function getSign($data, $key)
    {
        ...
        payLog('xinbao.txt',$key. "+++++259++". $sign_str);
    ...
    public function retSigned($data,$key,$sign)
    {
        ...
        $sign_str = rtrim($sign_str,"&");
        LgErr2('xinbao.txt',print_r($sign_str,true). "+++++".$key);
        $key = openssl_get_publickey($key);
        $flag = openssl_verify($sign_str,$sign,$key,OPENSSL_ALGO_MD5);
        LgErr2('xinbao.txt',$flag."====327");
        return $flag;
    }
