...
class HuanQiuPay extends PayInfo
{
    //请求接口Url

    public $url = 'http://huanqiu.comsj.com/Pay_Index.html';               //扫码正式（线上）调用接口
    public $h5_url = 'http://huanqiu.comsj.com/Pay_Index.html';
    public $payName = '环球支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            'pay_memberid'      => intval($config['merchantID']),
        ...
        $post_data['pay_productname'] = 'recharge';
        $post_data['pay_attach'] = 'huanqiu_'.$data['payment_id'];

        payLog('huanqiu.txt',print_r($post_data,true));


        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {

            $curlData = $this->httpPost($post_data, $this->url);
            payLog('huanqiu.txt',print_r($curlData,true).'---84--');
            //接口调用成功与否
            if ($curlData['code'] != 200) {
                $this->retArr['code'] = 275073;
                $this->retArr['msg']  = '支付调用失败！';
                payLog('huanqiu.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

                return $this->retArr;
            }
            $returnData = $curlData['data'];
            payLog('huanqiu.txt', print_r($returnData, true).'----113---');
            if ($returnData['status'] == "error") {
                $this->retArr['code'] = 275074;
                $this->retArr['msg']  = '支付接口调用失败';
                payLog('huanqiu.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));
...
//            payLog('huanqiu.txt',$url. "   ====137");
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('huanqiu.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('huanqiu.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 275020;
            payLog('huanqiu.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['returncode'] != '00') {
            ...
            payLog('huanqiu.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
        ...
        payLog('huanqiu.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 275022;
            payLog('huanqiu.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
...
//        payLog('huanqiu.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
    ...
    public function httpPost($data, $url)
    {
        ...
        payLog('huanqiu.txt',print_r($response,true). "+++251".$url);
...
//        array_filter($data);
        ...
        $sign_str .= "key=".$key;
        payLog('huanqiu.txt',$sign_str. "+++++++271");
    ...
    public function retSigned($data,$key,$sign)
    {
        ...
        payLog('huanqiu.txt',$sign_str . "+++++300");
