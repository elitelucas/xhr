...
class payTong extends PayInfo
{
    //请求接口Url

    public $url = 'https://gateway.paytongpay.com/pt_v3_gate/pay';               //扫码正式（线上）调用接口
    public $wy_url = 'https://gateway.paytongpay.com/pt_v3_quick/pay';
    public $payName = 'paytong支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        payLog('paytong.txt',print_r($post_data,true). "===92===");
        $curlData = $this->httpPost($post_data, $this->url);
        payLog('paytong.txt',print_r($curlData,true). "=====91==");
...
//                'html'  => $this->httpHtml($post_data, $this->url)
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('paytong.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('paytong.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 277020;
            payLog('paytong.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['tradeStatus'] != "SUCCESS") {
            ...
            payLog('paytong.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
        ...
        if($retSign != $sign){
            ...
            payLog('paytong.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));

            return $this->orderInfo;
        }

        $this->orderInfo['code']      = 0;
        $this->orderInfo['order_no']  = $data['outTradeNo'];  //商户订单号
        $this->orderInfo['amount']    = $data['orderPrice'];
        $this->orderInfo['serial_no'] = $data['trxNo'];  //平台订单号
