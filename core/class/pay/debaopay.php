...
class debaopay extends PayInfo
{
    //请求接口Url

    public $url = 'https://api.yuanruic.com/gateway/api/scanpay';               //扫码正式（线上）调用接口
    public $h5_url = 'https://api.debaozhifu.com/gateway/api/h5apipay';
    public $b2c = 'https://pay.yuanruic.com/gateway?input_charset=UTF-8';
    public $payName = '得宝支付';   //接口名称
...
//        $payCode = isset($config['payType'][$data['pay_type']]['payCode'])?$config['payType'][$data['pay_type']]['payCode']:'';
        ...
        if ($payStr == "direct_pay") {
            $url = $this->b2c;
            $post_data['interface_version'] = "V3.0";
            $post_data['input_charset'] = "UTF-8";
        }
//        if($payCode) $post_data['pay_type'] = $payCode;
//        if(isset($data['bank_code'])) $post_data['bank_code'] = $data['bank_code'];

        $post_data['sign'] = $this->getSign($post_data, $config['merchantPrivateKey']);
        $post_data['sign_type'] = 'RSA-S';

        payLog('debao.txt',print_r($post_data,true));

        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {

            $curlData = $this->httpPost($post_data, $url);
            payLog('debao.txt',print_r($curlData,true));
            //接口调用成功与否
            if ($curlData['code'] != 200) {
                $this->retArr['code'] = 277073;
                $this->retArr['msg']  = '支付调用失败！';
                payLog('debao.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

                return $this->retArr;
            }
            $returnData = $curlData['data']['response'];
            if ($returnData['result_code'] != 0) {
                $this->retArr['code'] = 277074;
                $this->retArr['msg']  = $returnData['result_desc'];
//                $this->retArr['msg']  = '支付接口调用失败';
                payLog('debao.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));
...
//            $url = urldecode($returnData['payURL']);
            ...
            $retData =  [
                ...
                'modes' => $data['pay_model'],
                'html'  => $this->httpHtml($post_data, $url)
            ];
            payLog('debao.txt',print_r($retData, 1). "   ====137");
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('debao.txt',print_r($postData['data'],true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('debao.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 277020;
            payLog('debao.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['tradeStatus'] != "SUCCESS") {
            ...
            payLog('debao.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
...
//        $data['merchantNo'] = $config['merchantID'];
        ...
        $retSign = $this->retSign($data, $config['merchantPublicKey'],$sign);
        payLog('debao.txt',$retSign.'----195--');
        if(!$retSign){
            $this->orderInfo['code'] = 277022;
            payLog('debao.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
...
//        payLog('debao.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
...
//        payLog('debao.txt',print_r($response,true). "+++251");
    ...
    public function retSign($data, $key ,$sign)
    {
        $sign_str = '';
        array_filter($data);
        ksort($data);
