...
class juzipay extends PayInfo
{
    //请求接口Url

    public $url = 'http://neilgu.xyz/pay?';               //扫码正式（线上）调用接口
    public $wy_url = 'http://neilgu.xyz/pay?';
    public $payName = '橘子支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            ...
            'order_id'      => $orderInfo."_".$data['payment_id'],
            'amount'      => floatval($data['money']),
            'pay_method'      => $payStr,
//            'return_url'      => "https://".$callbackurl,
            'return_url'      => "https://".$callbackurl."/?m=web&c=recharge&a=rechargeOk&order_sn=" . $orderInfo,
//            'notify_url'      => "https://".$callbackurl."/rechargeNotify.php",
        ...
        payLog('juzi.txt',print_r($post_data,true));
//        $url = $this->url. http_build_query($post_data);


        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {

            $curlData = $this->httpPost($post_data, $this->url);
            payLog('juzi.txt',print_r($curlData,true));
            //接口调用成功与否
            if ($curlData['code'] != 200) {
                $this->retArr['code'] = 277073;
                $this->retArr['msg']  = '支付调用失败！';
                payLog('juzi.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

                return $this->retArr;
            }
            $returnData = json_decode($curlData['data'],true);
//        payLog('juzi.txt', print_r($returnData, true).'----113---');

            if ($returnData['status'] != "0000") {
                $this->retArr['code'] = 277074;
                $this->retArr['msg']  = '支付接口调用失败';
                payLog('juzi.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));
...
//            payLog('juzi.txt',$url. "   ====137");
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('juzi.txt',print_r($postData['data'],true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('juzi.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 277020;
            payLog('juzi.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
...
//            payLog('juzi.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
//
//            return $this->orderInfo;
//        }
...
//        $data['merchantNo'] = $config['merchantID'];
        ...
        payLog('juzi.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 277022;
            payLog('juzi.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
...
//        payLog('juzi.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
...
//        payLog('juzi.txt',print_r($response,true). "+++251");
//        $response = json_decode(json_encode(simplexml_load_string($response)),true);
    ...
    public function getSign($data, $key)
    {
        ...
        $sign_str = sprintf("merchant_id=%s&order_id=%s&amount=%s&sign=%s",
            $data['merchant_id'],$data['order_id'],$data['amount'],$key);
