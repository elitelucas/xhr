...
class XinHuiPay extends PayInfo
{
    //请求接口Url
    //public $url = '';               //扫码测试调用接口
    public $url = 'http://spdo.zjwee.cn/Online/GateWay';       //扫码正式（线上）调用接口
    public $bank_url = 'http://spdo.zjwee.cn/Online/GateWay';  //第三方网银接口
    public $payName = '信汇支付';   //接口名称
    ...
    public $orderInfo = [            //异步验签结果返回格式
        'code' => 1,                 //0：数据获取成功，其他数字，数据获取失败
        'bank_num' => 245050,        //银行区分号（不同支付的前三位不同）
        'order_no' => '',            //后台数据库支付订单号
        'amount' => 0,               //支付金额
        'ret_error' => 0,            //回调处理失败时，返回接口字符串
        'ret_success' => 'ok',       //回调处理成功时，返回接口字符串
        'bank_name' => '信汇支付',   //支付方式名称
