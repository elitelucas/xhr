...
class RenRenPay extends PayInfo
{
    //请求接口Url

    public $url = 'http://39.106.239.206/pay/unifiedOrder.htm';               //扫码正式（线上）调用接口
    public $h5_url = 'http://39.106.239.206/pay/unifiedOrder.htm';
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            ...
            'totalFee'      => number_format($data['money'],2,'.',''),
            'channeltype'      => $payStr,
...
payLog('renren.txt',print_r($post_data,true). "===86");

        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {

            $curlData = $this->httpPost($post_data, $this->url);
            payLog('renren.txt',print_r($curlData,true).'---84--');
            //接口调用成功与否
            if ($curlData['code'] != 200) {
                $this->retArr['code'] = 275073;
                $this->retArr['msg']  = '支付调用失败！';
                payLog('renren.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');

                return $this->retArr;
            }
            $returnData = json_decode($curlData['data'],true);
            payLog('renren.txt', print_r($returnData, true).'----113---');
            if ($returnData['retCode'] != 100) {
                $this->retArr['code'] = 275074;
                $this->retArr['msg']  = '支付接口调用失败';
                payLog('renren.txt', '（' . $this->retArr['code'] . '）' . '支付接口调用失败，' . print_r($returnData, true));
...
//            payLog('renren.txt',$url. "   ====137");
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('renren.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        payLog('renren.txt',print_r($data,true).'----160--');

        if (!is_array($config)) {
            $this->orderInfo['code'] = 275020;
            payLog('renren.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
...
//            payLog('renren.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
//
//            return $this->orderInfo;
//        }
        ...
        payLog('renren.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 275022;
            payLog('renren.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
        ...
        $this->orderInfo['order_no']  = $orderId;  //商户订单号
        $this->orderInfo['amount']    = $data['totalFee'];
        $this->orderInfo['serial_no'] = $data['outerTradeNo'];  //平台订单号
...
//        payLog('renren.txt',print_r($html,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
...
//        curl_setopt($ch, CURLOPT_HEADER, false);
        ...
        payLog('renren.txt',print_r($response,true). "+++251");
...
//        array_filter($data);
        ...
        $sign_str .= "key=".$key;
        payLog('renren.txt',$sign_str. "+++++++271");
