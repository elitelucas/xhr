...
class BaiQianPay extends PayInfo
{
    //请求接口Url

    public $url = 'https://bq.baiqianpay.com/webezf/web/?app_act=openapi/bq_pay/pay';               //扫码正式（线上）调用接口
    public $bank_url = 'https://bq.baiqianpay.com/webezf/web/?app_act=openapi/bq_pay/pay';  //第三方网银接口
    public $payName = '百钱支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            'X1_Amount'      => number_format($data['money'],2,'.',''),
            'X2_BillNo'      => $orderInfo,
            'X3_MerNo'      => $config['merchantID'],
//            'X4_ReturnURL'      => "https://".$_SERVER['HTTP_HOST'].'/rechargeNotify.php',
            'X4_ReturnURL'      => "https://".$_SERVER['HTTP_HOST']."/?m=api&c=recharge&a=rechargeNotify&payment_id=" . $data['payment_id'],
        );
        $post_data['X6_MD5info'] = $this->getSign($post_data, $config['merchantKey']);
        $post_data['X7_PaymentType'] = $payStr;
        $post_data['X9_ClientIp'] = ip();
        ...
        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {
            ...
            //用于安全验证返回url是否非法
            session::set('qrcode_url', $returnData['imgUrl']);
            session::set('pay_url', '');
            $retData =  [
                'type'     => 1,
                'code_url' => $returnData['imgUrl'],
                'pay_url'  => '',
                'order_no' => $orderInfo,
    ...
    public function doPaycallBack($postData)
    {
        ...
        $this->orderInfo['order_no']  = $data['BillNo'];  //商户订单号
        $this->orderInfo['amount']    = $data['Amount'];
        $this->orderInfo['serial_no'] = $data['BillNo'];  //平台订单号
...
//            'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/536.11 (KHTML, like Gecko) Chrome/20.0.1132.47 Safari/536.11');// 添加浏览器内核信息，解决403问题
    ...
    public function getSign($data, $key)
    {
        ...
        $sign_str = $sign_str.strtoupper(md5($key));
