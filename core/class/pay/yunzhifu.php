 ...
 * Created by Kevin.
 * @author kevin
 * @copyright HCHT 2018/1/10 10:30
 * @description 云支付
 */

include_cache(S_CORE . 'class' . DS . 'pay' . DS . 'payinfo' . '.php');

class YunZhiFuPay extends PayInfo
{
    //请求接口Url
    public $url = 'http://cashier.367zet.cn/cgi-bin/netpayment/pay_gate.cgi';       //扫码正式（线上）调用接口
    public $bank_url = 'http://cashier.367zet.cn/cgi-bin/netpayment/pay_gate.cgi';  //第三方网银接口
    public $payName = '云支付';   //接口名称
    ...
    public $orderInfo = [            //异步验签结果返回格式
        'code' => 1,                 //0：数据获取成功，其他数字，数据获取失败
        'bank_num' => 250050,        //银行区分号（不同支付的前三位不同）
        'order_no' => '',            //后台数据库支付订单号
        'amount' => 0,               //支付金额
        'ret_error' => 0,            //回调处理失败时，返回接口字符串
        'ret_success' => 'SUCCESS',  //回调处理成功时，返回接口字符串
        'bank_name' => '云支付',     //支付方式名称
    ...
    public function doPay($data)
    {
        ...
        $post_data = array(
            'apiName'      => "WEB_PAY_B2C",         // WAP方式：“WAP_PAY_B2C”（手机支付）WEB方式：“WEB_PAY_B2C”（pc浏览器）
            'apiVersion'   => "1.0.0.0",             // 商户API版本
