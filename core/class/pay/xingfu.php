...
class XingFuPay extends PayInfo
{
    //请求接口Url

    public $url = 'http://gate.lfbpay.com/cooperate/gateway.cgi';                 //扫码测试调用接口

    public $bank_url = 'http://gate.lfbpay.com/cooperate/gateway.cgi';  //第三方网银接口

    public $payName = '星付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        payLog('xingf.txt',print_r($post_data,true).'---92--');
        ...
        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {
            ...
            payLog('xingf.txt',print_r($curlData,true).'---93--');
            ...
            payLog('xingf.txt',print_r($returnInfo,true).'---97--');
            ...
            payLog('xingf.txt',print_r($retData,true).'---123--');
            $this->retArr['code'] = 0;
    ...
    {
        //处理post回调数据
        payLog('xingf.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        payLog('xingf.txt',print_r($data,true).'----149--');
        $payment_id = $postData['payment_id'];
        $config = unserialize($postData['config']);
        if (!is_array($config)) {
            $this->orderInfo['code'] = 243005;
            payLog('xingf.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['status'] != 1) {
            ...
            payLog('xingf.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
        ...
        if(strtoupper($retSign) != $data['sign']){
            ...
            payLog('xingf.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
