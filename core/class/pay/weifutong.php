...
class WeiFuTongPay extends PayInfo
{
    //请求接口Url

    public $url = 'https://pay.swiftpass.cn/pay/gateway';                 //扫码测试调用接口

    public $bank_url = 'http://qiezhifu.com/cashier';  //第三方网银接口

    public $payName = '威富通';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        if (!$orderInfo) {
            ...
            payLog('weifutong.txt', '（' . $this->retArr['code'] . '）' . $this->payName . '订单生成失败，' . print_r($data, true));
        ...
        if (empty($data['pay_type']) || empty($config['payType'][$data['pay_type']])) {
            ...
            payLog('weifutong.txt', '（' . $this->retArr['code'] . '）' . $this->payName . '银行类型不存在，' . print_r($data, true));
...
//        payLog('weifutong.txt',print_r($post_data,true).'---89--');
        $post_data['sign'] = $this->getSign($post_data, $config['merchantKey']);
        $post_data = $this->toXml($post_data);

        $curlData = $this->httpPost($post_data, $this->url);
        payLog('weifutong.txt',print_r($curlData,true).'---93--');
        //接口调用成功与否
        if ($curlData['code'] != 200) {
            $this->retArr['code'] = 249003;
            $this->retArr['msg']  = '支付调用失败！';
            payLog('weifutong.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--101--');
        ...
        if (!is_array($curlData['data'])) {
            ...
            $this->retArr['msg']  = '支付调用失败！';
            payLog('weifutong.txt', '（' . $this->retArr['code'] . '）' . '付支付接口调用失败，' . print_r($curlData, true).'--109--');
        ...
        if ($config['payType'][$data['pay_type']]['request_type'] == 1) {
            ...
            payLog('weifutong.txt',print_r($returnInfo,true).'---97--');
            if ($returnInfo['returnCode'] == 'FAIL') {
                $this->retArr['code'] = 249004;
                $this->retArr['msg']  = '支付调用失败！';
                payLog('weifutong.txt', '（' . $this->retArr['code'] . '）' . $this->payName . '付接口调用失败，' . print_r($returnInfo, true));
                return $this->retArr;
            }
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('weifutong.txt',print_r($data,true).'----149--');
        $payment_id = $postData['payment_id'];
        $config = unserialize($postData['config']);
        if (!is_array($config)) {
            $this->orderInfo['code'] = 249005;
            payLog('weifutong.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));
        ...
        if ($data['status'] != 1) {
            ...
            payLog('weifutong.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
