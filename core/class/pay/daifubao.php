...
/**
 *	Author: Kevin
 * 	CreateDate: 2018/7/5 16:45
 *  description: 代付宝支付
 */
include_cache(S_CORE . 'class' . DS . 'pay' . DS . 'payinfo' . '.php');

class DaiFuBaoPay extends PayInfo
{
    //请求接口Url
    public $url = 'https://gateway.shlangdi.com/controller.action ';  //部署
    public $retArr = [
        'code' => 1,
        'msg' => '',
        'data' => []
...
//            'pg_BankCode'  => '',
        ...
        $post_data['ph_Ip'] = ip();
        $post_data['pi_Url'] = "https://".$_SERVER['HTTP_HOST']."/?m=web&c=recharge&a=rechargeOk&order_sn=" . $orderInfo;

        $retStr = $this->toUrlParamsStr($post_data);
        $post_data['hmac'] = $this->HmacMd5($retStr, $config['merchantKey']);

//            log_to_mysql($post_data, 'daifubao.doPay.post_data');
        $curlData = $this->httpPost($this->url, $post_data);

        payLog('dfb.txt',print_r($curlData,true). "===103++");
        die;

        //网银支付调用
        if ($data['pay_type'] == 'wy' || $data['pay_type'] == 'wyh') {

//            log_to_mysql($curlData, 'daifubao.doPay.curlData');
            if (json_decode($curlData['data'], true)) {
                $this->retArr['code'] = 275004;
            ...
            //curl接口
            $curlData = $this->httpPost($this->url, $post_data);
            payLog('dfb.txt',print_r($curlData,true). "===131++");

            //接口调用成功与否
            if ($curlData['code'] != 200) {
                $this->retArr['code'] = 275005;
            ...
            if (!isset($retData['r1_Code']) || $retData['r1_Code'] != 1) {
                $this->retArr['code'] = 275006;
                $this->retArr['msg']  = '支付二维码生成失败！';
                payLog('payerror.log', '（' . $this->retArr['code'] . '）' . '支付接口调用返回信息成功，但返回结果中二维码信息获取失败' . print_r($retData, true));
            ...
            if (empty($retOderPayNo) || empty(retOderPayQrcodrUrl)) {
                $this->retArr['code'] = 275008;
                $this->retArr['msg']  = '支付二维码生成失败！';
                payLog('payerror.log', '（' . $this->retArr['code'] . '）' . '支付返回数据错误，' . print_r($retData, true));
            ...
            if (empty($result)) {
                $this->retArr['code'] = 275009;
                $this->retArr['msg']  = '支付二维码生成失败！';
                payLog('payerror.log', '（' . $this->retArr['code'] . '）' . '支付返回数据成功,但订单表没有查到相应未完成的订单号，' . print_r($retData, true));
    ...
    public function doPaycallBack($postData)
    {
        ...
        $retSign = $this->HmacMd5($hmacStr, $config['merchantKey']);
        if($retSign != $hmac){
            $this->orderInfo['code'] = 275012;
            payLog('payerror.log', '（' . $this->orderInfo['code'] . '）' . '支付返回数据成功，但验证签名失败！' . print_r($data, true));
