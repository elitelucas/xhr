...
class WanFuBao extends PayInfo
{
    //请求接口Url

    public $url = 'https://pay.allinepay.com/pay.html';                 //扫码测试调用接口

    public $bank_url = 'https://pay.allinepay.com/pay.html';
    public $payName = '万付宝支付';   //接口名称
    ...
    public function doPay($data)
    {
        ...
        payLog('wfb.txt',print_r($config,true). "+++65");
        if (empty($data['pay_type']) || empty($config['payType'][$data['pay_type']])) {
            $this->retArr['code'] = 270072;
        ...
        $post_data = array(
            ...
            'callbackurl'      => "https://".$callbackurl."/paycallback.php",
        );
        $post_data['sign'] = $this->getSign($post_data, $config['merchantKey']);
        $post_data['attach'] = "wanfubao_".$data['payment_id'];
        $post_data['isshow'] = 1;
    ...
    public function doPaycallBack($postData)
    {
        ...
        payLog('wfb.txt',print_r($postData,true).'----157--');
        parse_str($postData['data'],$data);
        payLog('wfb.txt',print_r($data,true).'----149--');
        //D('accountrecharge')->save(['verify_remark' => print_r($data, true)], ['id' => 6]);

        $config = unserialize($postData['config']);
        if (!is_array($config)) {
            $this->orderInfo['code'] = 270020;
            payLog('wfb.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,获取数据库配置错误！'  . print_r($data, true));

            return $this->orderInfo;
        }

        if ($data['orderstatus'] != 1) {
            $this->orderInfo['code'] = 270021;
            payLog('wfb.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知：返回信息不是充值成功的信息，出现错误！'  . print_r($data, true));
        ...
        payLog('wfb.txt',$retSign.'----195--');
        if($retSign != $sign){
            $this->orderInfo['code'] = 270022;
            payLog('wfb.txt', '（' . $this->orderInfo['code'] . '）' . $this->payName . '异步通知,验签失败！'  . print_r($data, true));
        ...
        $this->orderInfo['serial_no'] = $data['ordernumber'];
    ...
    public function httpHtml($post_data, $url)
    {
        ...
        payLog('wfb.txt',print_r($post_data,true). "  ==220== ");
        //var_dump($html);
        return $html;
    }
    ...
    public function getSign($data, $key)
    {
        if (!$data['attach']) {
            $signature = sprintf(
                "partner=%s&banktype=%s&paymoney=%s&ordernumber=%s&callbackurl=%s",
                $data['partner'], $data['banktype'], $data['paymoney'], $data['ordernumber'], $data['callbackurl']);
        } else {
            $signature = sprintf(
                "partner=%s&ordernumber=%s&orderstatus=%s&paymoney=%s",
                $data['partner'], $data['ordernumber'], $data['orderstatus'], $data['paymoney']);
        }
//        LgErr2('wfb.txt',$signature.$key. " +++++++269");
