...
class weixin{
	...
	public function getRevData(){
		$this->log("微信服务器发送内容___".var_export($this->_receive, TRUE));
	...
	public function createMenu($data){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
		$this->log($result);
		if($result){
	...
	public function getMenu(){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::MENU_GET_URL.'access_token='.$this->access_token);
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function deleteMenu(){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::MENU_DELETE_URL.'access_token='.$this->access_token);
		$this->log($result);
		if($result)
		{
			$jsonArr = json_decode($result, true);
	...
	public function getUserInfo($openid){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->access_token.'&openid='.$openid);
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function getUserList($next_openid=''){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::USER_GET_URL.'access_token='.$this->access_token.'&next_openid='.$next_openid);
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function getGroup(){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::GROUPS_GET_URL.'access_token='.$this->access_token);
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function getUserGroup($openid){
		if(!$this->access_token && !$this->checkAuth()) return false;
		$result = self::Http_Get(self::API_URL_PREFIX.'/groups/getid?access_token='.$this->access_token,self::jsonEncode(array('openid'=>$openid)),'post');
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function createGroup($name){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::GROUPS_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function updateGroup($id, $name){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::GROUPS_UPDATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function updateGroupMembers($openid, $to_groupid){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::GROUPS_MEMBERS_UPDATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function sendCustomMessage($touser, $data, $msgType = 'text'){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::MESSAGE_CUSTOM_SEND_URL.'access_token='.$this->access_token, $this->jsonEncode($arr), 'post');
		if($result){
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
			else return true;
	...
	public function checkAuth(){
		...
		$access_token = F($cache_flag);
		if($access_token){
			$this->access_token = $access_token;
			$this->log('cache token___'.$this->access_token);
			return true;
		}
		// 请求微信服务器获取access_token
		$result = self::Http_Get(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$this->appid.'&secret='.$this->appsecret);
		if($result){
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)){
				...
				$this->log('new token___'.$this->access_token);
				$expire = isset($jsonArr['expires_in']) ? intval($jsonArr['expires_in'])-100 : 3600;
				// 将access_token保存到缓存中
				F($cache_flag,$this->access_token,$expire);
				return true;
			}
		}
		return false;
	...
	public function getSnsAccessToken($code){
		$result = self::Http_Get(self::SNS_OAUTH_ACCESS_TOKEN_URL.'appid='.$this->appid.'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');
		if($result){
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
			else return $jsonArr;
	...
	public function refershToken($refresh_token){
		$result = self::Http_Get(self::SNS_OAUTH_REFRESH_TOKEN_URL.'appid='.$this->appid.'&grant_type=refresh_token&refresh_token='.$refresh_token);
		if($result){
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
			else return $jsonArr;
	...
	public function getSnsUserInfo($access_token, $openid){
		$result = self::Http_Get(self::SNS_USERINFO_URL.'access_token='.$access_token.'&openid='.$openid);
		if($result){
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
			else return $jsonArr;
	...
	public function createQrcode($scene_id, $type=0, $expire=1800){
		...
		$result = self::Http_Get(self::API_URL_PREFIX.self::QRCODE_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
		if($result)
		{
			$jsonArr = json_decode($result, true);
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
	...
	public function error($data){
		...
		file_put_contents(S_CACHE."log".DS.'weixin.log','WEIXIN API errcode:['.$this->errCode.'] errmsg:['.$this->errMsg."]\n",FILE_APPEND);
	}
	...
	public function valid(){
		if($this->checkSignature()) exit($_GET['echostr']);
	}
	/**
	 * 连接
	 *
	 ...
	 * @return unknown
	 */
	private function Http_Get($url,$data=array(),$method='GET'){
		$this->httprequest->connect($url,$method)->send($data);
		if($this->httprequest->status() == 200){
			$str = $this->httprequest->body();
			return $str;
		}elseif ($this->httprequest->status() == 601){
			$this->error(array('errcode'=>1000601,'errmsg'=>'连接超时'));
		}else {
			$this->error(array('errcode'=>1000602,'errmsg'=>'连接失败'));
		}
	}
	
	//trace weixin log
	private function log($c){
		$log_file = str_replace('core/class','',str_replace('\\','/',realpath(dirname(__FILE__).'/'))).'caches/log/weixin/'.date('Ymd');
