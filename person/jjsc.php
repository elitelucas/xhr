...
//清楚过期的数据
$sql = "delete from un_bet_list where lottery_type = 9 and bet_time < ".time();
$rs = $personDb->query($sql);
lg("auto_person","删除急速赛车已经执行过的投注信息执行结果->".var_export($rs,true));
...
if(!empty($rows)) {
    foreach($rows as $value) {
        ...
        if($config['lottery_type'] == 9){
            lg("auto_person","获取急速赛车开启中的机器人配置->".var_export($value,true)."->开始生成数据");
                ...
                for($a=1;$a<=$count;$a++) {
                    ...
                    if($config['money']['type'] == 1) {
                        foreach($config['ids'] as $v) {
                            ...
                            if ($g == 5){
                                //如果是猜冠亚，则组装玩法
                                $a1 = array_rand($wayArr[$g]);
                                $a2 = array_rand($wayArr[$g]);
                                $bet['way'] = "冠亚_{$a1}_{$a2}";
                            }
