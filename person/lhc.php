...
//清楚过期的数据
$sql = "delete from un_bet_list where lottery_type = 7 and bet_time < ".time();
$rs = $personDb->query($sql);
lg("auto_person","删除六合彩已经执行过的投注信息执行结果->".var_export($rs,true));
...
if(!empty($rows)) {
    foreach($rows as $value) {
        ...
        if($config['lottery_type'] == 7){
            lg("auto_person","获取六合彩开启中的机器人配置->".var_export($value,true)."->开始生成数据");
