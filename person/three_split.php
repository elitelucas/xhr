...
//清楚过期的数据
$sql = "delete from un_bet_list where lottery_type = 6 and bet_time < ".time();
$rs = $personDb->query($sql);
lg("auto_person","删除三分彩已经执行过的投注信息执行结果->".var_export($rs,true));

//获取机器人配置列表
$rows = $personDb->getall("select * from un_person_config where state = 1 and (type = 1 OR type = 3)");
lg("auto_person","获取机器人配置->".var_export($rows,true));

//添加机器人注单
if(!empty($rows)) {
    foreach($rows as $value) {
        $config = json_decode($value['value'],true);
        if($config['lottery_type'] == 6){
            lg("auto_person","获取三分彩开启中的机器人配置->".var_export($value,true)."->开始生成数据");
