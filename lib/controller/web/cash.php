...
class CashAction extends Action{
    ...
    public function getBankCard(){
        ...
        //银行列表
        $res = $this->model->getBindBank($this->userId);
        if(empty($res))
        {
            $bank = 1;
        }
        ...
        $field = 'paypassword';
        $userInfo = $this->model3->getOneCoupon($field, $where);
        $is_set_paypassword = $userInfo['paypassword'] ? 1 : 0;

        //判断用户是否存在处理中提现
        $cashArr = $this->model2->getOneCoupon('id,status,is_read', array('user_id' => $this->userId), 'addtime DESC');
//        payLog('tixian.txt',print_r($cashArr,true). "+++76");

        if ($cashArr['status'] === '0' || $cashArr['is_read'] === '0') {
            //跳转到详情界面
            header("Location:" . url('','','presentState',array('cash_id' => $cashArr['id'])));
        }

        $JumpUrl = $this->getUrl();
        include template('wallet/withdrawalsFlow');
    }
    ...
    public function cash(){
        ...
        if(($money<$cash['cash_lower']) || ($money>$cash['cash_upper'])){
            ErrorCode::errorResponse(100026, 'Withdrawal amount '.$cash['cash_lower'].' to '.$cash['cash_upper']);
        }
        ...
        $field = 'id';
        lg('cash_debug',"uid::{$this->userId},where::".encode($where));
        $accountCash = $this->model2->getOneCoupon($field, $where);
        lg('cash_debug',"uid::{$this->userId},accountCash::".encode($accountCash));
		...
		if(in_array($userInfo['reg_type'],array(8,9,11))){
        ...
        if($paypsd !== $userInfo['paypassword']){
            ...
            if($paypsd_error>=3){
                ...
                $num =3 - $paypsd_error;
                ErrorCode::errorResponse(100018 ,'The fund password is wrong, you still have '.$num.' opportunities');
            }
        ...
        lg('cash_limit_debug.json','H5用户::'.$this->userId.',提现设置$nRt::'.$nRt);
        if (!empty($nRt)) { //倍率已设置  并且大于0 才做判断
            //验证提现限制
            $time = $this->db->result("select addtime from un_account_cash where user_id = {$this->userId} and (status = 1 or status = 4) order by id desc");
            if (empty($time)) {
            ...
            $sql = "select bet_amount from un_user where id = {$this->userId}";
            $bet_amount = $this->db->result($sql);

            lg('cash_limit_debug.json','H5用户::'.$this->userId.',有效投注::'.$betRt.',sql::'.$sql);
            ...
            lg('cash_limit_debug.json','H5用户::'.$this->userId.',充值总额::'.$regRt.',sql::'.$sql);
            lg('cash_limit_debug.json','H5用户::'.$this->userId.',提现设置$nRt::'.$nRt.',有效投注::'.$betRt.',充值总额::'.$regRt.'$regRt * $nRt > $betRt::'.($regRt * $nRt > $betRt));
            ...
            //查最后一次充值时间
            $sql = "SELECT `addtime`,money FROM `un_account_recharge` WHERE user_id = {$this->userId} AND (STATUS = 1 or status = 4) ORDER BY `addtime` DESC LIMIT 1";
            $re = $this->db->getone($sql); //最后一次时间
            $last_time= isset($re['addtime']) ? $re['addtime'] : $time;
            $last_money = isset($re['money']) ? $re['money'] : 0;
            lg('cash_limit_debug.json','H5最后充值验证,最后时间::'.$last_time.',最后金额::'.$last_money.',SQL::'.$sql);
            ...
            lg('cash_limit_debug.json','H5最后充值验证,用户::'.$this->userId.',最后充值到当前时间的有效投注::'.$lbetRt.',sql::'.$sql);
            lg('cash_limit_debug.json','H5最后充值验证,用户::'.$this->userId.',提现设置::'.$nRt.',最后充值到当前时间的有效投注::'.$lbetRt.'$lbetRt < $last_money * $nRt::'.($lbetRt < $last_money * $nRt));
            ...
            if($lbetRt < ($last_money * $nRt)+$bet_amount){
                ...
                if($agree==0){
                    if($current_money>0){
                        ErrorCode::errorResponse(100039, "Since your last deposit did not complete the entered amount, you can only withdraw (".$current_money.") USD now. Do you want to continue with the withdrawal?");
                    }else{
        ...
        $preventFlag = 'cashCashUserId' . $this->admin['userid'];
        if (preventSupervene($preventFlag, 10)) {
            ErrorCode::errorResponse(100026,'Please re-initiate withdrawal after 10 seconds');
        }
...
//        dump($wi)
        ...
        try{
            ...
            $logData = array(
                ...
                'remark' => 'User: '.$userInfo['username'].' Apply for withdrawal: '.$money.' Bound bank id: '.$bankId.' Account: '.$user_bank['account'],
                'addtime' => SYS_TIME
            );
...
//            }
            ...
            if ($data['status'] != 6) {
                ...
                if (!empty($bankArr) && $money > $bankArr['balance']) { //余额不足报警
                    ...
                    $data['type'] = 4;
                    $data['remark'] = "User id: {$this->userId}, withdrawal, Withdrawal amount: {$money}, Cash card{$bankArr['name']} is not enough, Remaining:{$bankArr['balance']}";
...
//                }
    ...
    public function detail(){
        ...
        include template('wallet/cashDatails');
    }
    ...
    public function getWithdrawSet($user_id)
    {
        ...
        if ($freeCont['value'] > 0) {
            ...
            $sql = 'select id from un_account_cash where user_id = ' . $this->userId . ' and status in (0,1,4,5,6) and addtime between ' . $start_time . ' and ' . $end_time . ' limit ' . $freeCont['value'];
    ...
    public function presentState(){
        ...
        $sql = "SELECT c.id, c.order_sn, c.addtime, c.money, c.bank_id, c.status AS type,c.credited, c.verifytime, c.verifyremark, b.id as bid, b.name, b.account, b.bank, b.branch FROM un_account_cash AS c LEFT JOIN un_user_bank AS b ON b.id = c.bank_id WHERE c.id = {$id} and b.user_id = {$this->userId}";
        $res = $this->model2->getOneData($sql);
//payLog('tixian.txt',print_r($res,true). "--475");
        if(in_array($res['type'],array('1','2','3','4'))){
            //变更读取数据
            $map = array(
                'is_read' => 1
            );
        ...
        if ($res['type'] == 5 || $res['type'] == 6 || $res['type'] == 8) {
            $res['type'] = 0;
        }
        if ($res['type'] == 4) {
            $res['type'] =1;
        ...
        //银行列表
        $bank = $this->getBank();
        $res['bank_name'] = $bank[$res['bank']];
        $tempArr = json_decode($res['verifyremark'], true);
        $res['verifyremark'] = $tempArr['remark'][0]['remark'];
        
        include template('wallet/withdrawalsDatails');
    }
