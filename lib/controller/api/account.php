...
class AccountAction extends Action {
    ...
    public function betting_money()
    {
        if($_REQUEST['pass'] != 'e10adc3949ba59abbe56e057f20f883e'){
            ErrorCode::errorResponse(200003, 'Incorrect password and no access');
            return false;
        }
        ...
        if(!empty($start_date) && !empty($end_date)){
            ...
            $msgTime = 'Time limit: '.$start_date.' to '.$end_date." 23:59:59";
        }elseif(!empty($start_date)){
            $start_time = strtotime($start_date);
            $where = " AND l.addtime >= {$start_time}";
            $msgTime = 'Time is greater than or equal to: '.$start_date;
        }elseif(!empty($end_date)){
            $end_time = strtotime($end_date." 23:59:59");
            $where = " AND l.addtime <= {$end_time}";
            $msgTime = 'Time is less than or equal to: '.$end_date." 23:59:59";
        }else{
            $where = "";
            $msgTime = 'All data: ';
        }
        ...
        echo $msgTime;
        echo "<br>";
        echo '<span style="color: #4dcd70">Total bet:</span> '.$arr['betting_money'].' USD';
        echo "<br>";
        echo '<span style="color: #4dcd70">Total wins:</span> '.$arr['award_money'].' USD';
        echo "<br>";
        echo '<span style="color: #4dcd70">Total betting profit:</span> '.$arr['betting_profit_money'].' USD';
        echo "</div><br>";
        return false;
    }
    ...
    public function index() {
        ...
        $field = 'id';
        $userBank = $this->model3->getOneCoupon($field, $where, 'id desc');
        $res['is_banded_bank'] = empty($userBank) ? 0 : 1;

        //资金密码
        $where = array(
            'id' => $this->userId
        );
        $field = 'paypassword,reg_type,email';
        $userInfo = $this->model2->getOneCoupon($field, $where);
        $res['is_set_paypassword'] = $userInfo['paypassword'] ? 1 : 0;
        $res['is_tourist'] = 2;
        $res['email'] = $userInfo['email'];
        if(in_array($userInfo['reg_type'],array(0,8,9))){
            $res['is_tourist'] = 1;
        }
        ...
        $field = 'id,status,is_read';
        $order = 'id DESC';
        $accountCash = $this->model4->getList($field,$where,$order,1);

        if(!empty($accountCash)){
            if($accountCash[0]['is_read'] == 1){
                $res['cash_id'] = '0';
                $res['cash_status'] = '0';
            }else{
                $res['cash_id'] = $accountCash[0]['id'];
                $res['cash_status'] = $accountCash[0]['status'];
            }
        }else{
            $res['cash_id'] = '0';
            $res['cash_status'] = '0';
    ...
    /**
     * 获取当前实时余额
     * @method get /index.php?m=api&c=account&a=getCurrentUserMoney&token=b5062b58d2433d1983a5cea888597eb6
    ...
    public function getCurrentUserMoney()
    {
        ...
        $field = 'money';
        $rt_data = $this->getOneAccount($where, $field);
        ErrorCode::successResponse((array)$rt_data);
    }

    /**
     * 交易记录
     * @method get /index.php?m=api&c=cash&a=cashList&token=b5062b58d2433d1983a5cea888597eb6&bank_id=1&money=1&psd=123
     * @param
     * @return
     */
    public function getBills() {
        //验证参数
        $this->checkInput($_REQUEST, array('token', 'start_time', 'end_time', 'type', 'page'));
        //验证token
        $this->checkAuth();

        $user_id = getParame('user_id',0,0,'int');
        if(!$user_id) $user_id = $this->userId;

        $source = getParame('source',0,1,'int');            //1交易记录   2团队会员交易记录（）
        ...
        //交易类型列表 app 返回格式
        $trantypeName = array();
        foreach ($trantype['lists'] as $k => $v) {
            $trantypeName[$k] = array("id"=>$k,"name"=>$v);
        }
        //交易类型列表
        $trantypes = array(
            array("id"=>0,"name"=>'全部'),
            array("id"=>1,"name"=>'充值'),//充值：充值
            array("id"=>2,"name"=>'提现'),//提现：冻结，成功，失败
            array("id"=>3,"name"=>'投注'),//投注：中奖，撤单，未中奖，和局
            array("id"=>4,"name"=>'返水'),//反水：自身，直接，团队
            array("id"=>5,"name"=>'返利'),//返利：返利赠送，分享返现
            array("id"=>6,"name"=>'其他'),//其他：额度调整，手续费
        );
        if($source == 2){
            $trantypes = array(
                array("id"=>0,"name"=>'全部'),
        ...
        switch ($_REQUEST['type']){
            ...
            case 5://返利：返利赠送，分享返现
                $types = "18,66";
                break;
            case 6://其他：额度调整，手续费，提现手续费，大转盘活动赠送元宝彩金,博饼活动赠送元宝彩金,圣诞活动赠送元宝彩金,红包活动,九宫格
                $types = "32,48,154,1000,999,998,997,995,994,993,992,1071";
                break;
            default://全部
                if($source == 1) $types = "";
                if($source == 2) $types = "10,12,13,14,120,301";
        }
        ...
        $count = $this->model->getBillsCnt($where);
        $pageNum = ceil($count / $pageCnt);
        ...
        //显示+ -
        $arr = array(10, 12, 14, 18, 19, 20, 21, 51, 66, 1000, 999, 998, 997, 995, 994, 993, 992, 301,1071);// + 充值,中奖,撤单,返利赠送,自身返水,直接会员返水,团队返水,提现失败,分享返现,和局
        $arr2 = array(11,13, 25, 48,120, 154);//- 提现成功,投注,提现冻结,手续费,回滚, 提现手续费
        //32,会员额度调整 大于0 + 小于0 -
...
//                $single_money[$v['type']]['type'] = $v['type'];
//                $single_money[$v['type']]['money'] += $lists[$k]['money'];
                if($v['type'] == 10){
                    //$sql = "SELECT payment_id FROM `un_account_recharge` WHERE `order_sn` = '{$v['order_num']}' AND `user_id` = {$this->userId}";
                    //$payid = O('model')->db->result($sql);
                    //$lists[$k]['pay_type'] = $payid?$pay[$payid]['name']:'银联';
                    $sql = "SELECT payment_id, bank_name FROM `un_account_recharge` WHERE `order_sn` = '{$v['order_num']}' AND `user_id` = {$user_id}";
                ...
                elseif(in_array($v['type'],array(12,13,301))){
                    $sql = "SELECT issue FROM `un_orders` WHERE `order_no` = '{$v['order_num']}' AND `user_id` = {$user_id}";
                    $issue = O('model')->db->result($sql);
                    $lists[$k]['issue'] = $issue;
                }
                elseif($v['type'] == 14)
                {
                    $sql = "SELECT remark FROM `un_account_log` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id}";
                    $issue = O('model')->db->result($sql);
                    if(!empty($issue))
                    {
                        $xxx = explode(" ",$issue);
                ...
                //大转盘在状态处显示几等奖
                elseif($v['type'] == 1000){
                    $sql = "SELECT prize_id FROM `un_turntable_award_log` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id}";
                    ...
                    //博饼活动
                    $sql = "SELECT ranking,event_num FROM `un_activity_prize` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id} AND activity_type = 1";
                    ...
                    //双旦活动
                    $sql = "SELECT prize_project,event_num FROM `un_activity_prize` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id} and activity_type = 2";
                    $a = O('model')->db->getone($sql);
                    $lists[$k]['status_txt'] = "第 ".$a['event_num']." 期 获得".$a['prize_project'];

                } elseif($v['type'] == 997) {

                    //红包活动
                    $sql = "SELECT remark,activity_stage FROM `un_redpacket_gain_log` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id}";
                    ...
                    //九宫格活动
                    $sql = "SELECT prize_project,event_num FROM `un_activity_prize` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id} and activity_type = 3";
                    $a = O('model')->db->getone($sql);
                    $lists[$k]['status_txt'] = "第 ".$a['event_num']." 期 获得".$a['prize_project'];
                    ...
                    //平台任务
                    $sql = "SELECT remark FROM `un_task_prize` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id}";
                    $a = O('model')->db->getone($sql);
                    ...
                    //福袋活动
                    $sql = "SELECT prize_project,event_num FROM `un_activity_prize` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id} and activity_type = 4";
                    $a = O('model')->db->getone($sql);
                    $lists[$k]['status_txt'] = "第 ".$a['event_num']." 期 获得".$a['prize_project'];
                    ...
                    //刮刮乐活动
                    $sql = "SELECT prize_project,event_num FROM `un_activity_prize` WHERE `order_num` = '{$v['order_num']}' AND `user_id` = {$user_id} and activity_type = 5";
                    $a = O('model')->db->getone($sql);
                    $lists[$k]['status_txt'] = "第 ".$a['event_num']." 期 获得".$a['prize_project'];
                }
                elseif($v['type'] == 10650) {
...
//        dump($lists);
        ...
        if(!empty($type_opt)){
            ...
            foreach ($typeids as $v){
                $sql = "SELECT IFNULL(SUM(money),0) FROM un_account_log WHERE addtime BETWEEN {$start_time} and {$end_time} AND `type` = {$v} AND `user_id` = {$user_id}";
                $single_money[$v]['name'] = $trantypeName[$v]['name'];
                $single_money[$v]['money'] = $this->db->result($sql);
            }
        }
        ...
        switch ($type_opt){
            ...
            case 5://返利：返利赠送，分享返现
                ...
                break;
            case 6://其他：额度调整(32)，手续费(48)，提现手续费(154)，大转盘(1000)，博饼(999)，双旦(998)，红包(997)，九宫格(995)，平台任务(994)，福袋(994)，刮刮乐(994)
                $types = "32,48,154,1000,999,998,997,995,994,993,992,1073";
        ...
        array_multisort(array_column($data['list'],'addtime'),SORT_DESC,$data['list']);
    ...
    public function details(){
        //验证参数
        $this->checkInput($_REQUEST, array('token','type', 'order_sn',), 'all');
        //验证token
        $this->checkAuth();
        ...
        if(!in_array($type,$trade['tranTypeIds'])){
            ErrorCode::errorResponse(100028,'No data of this type');
        }
        ...
        switch ($type){
            case 10://充值
                ...
                if(empty($res)){
                    ErrorCode::errorResponse(100029,'This data does not exist');
                }
                $data = array(
            ...
            case 13: //投注
                ...
                if (in_array($res['lottery_type'], array(2, 4))) {
                    $data = array(
                        array('name' => "流水号", 'value' => $res['order_num']),
                ...
                }elseif(in_array($res['lottery_type'], array(5, 6,7,8,10,11,13,14))){
...
//                );
            ...
            case 48://手续费
                ...
                /*$sql = "SELECT order_num, money, use_money, addtime FROM un_account_log WHERE type = 48 AND order_num = '".$order_sn."'";
                $res = O('model')->db->getOne($sql);
                if(empty($res)){
                    ErrorCode::errorResponse(100029,'This data does not exist');
                }
                $data = array(
                    array('name'=>"流水号",'value'=>$res['order_num']),
                    array('name'=>"交易时间",'value'=>date("Y-m-d H:i",$res['addtime'])),
                    array('name'=>"手续费用",'value'=>$this->convert($res['money'])." 元宝"),
                    array('name'=>"即时余额",'value'=>$this->convert($res['use_money'])." 元宝")
                );*/
                break;
            case 51://提现失败
