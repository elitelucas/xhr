...
class Gateway
{
    ...
    protected static function sendCmdAndMessageToClient($client_id, $cmd, $message, $ext_data = '')
    {
        ...
        lg('gateway_debug',var_export(array(
            '$client_id'=>$client_id,
            '$cmd'=>$cmd,
            '$message'=>$message,
            'Context::$client_id'=>Context::$client_id,
            '($client_id === Context::$client_id || $client_id === null)'=>($client_id === Context::$client_id || $client_id === null),
        ),1));
    ...
    protected static function sendBufferToGateway($address, $gateway_buffer)
    {
        ...
        if (self::$businessWorker) {
            lg('gateway_debug',var_export(array(
                ...
                '(!isset(self::$businessWorker->gatewayConnections[$address]))'=>(!isset(self::$businessWorker->gatewayConnections[$address])),
            ),1));
            if (!isset(self::$businessWorker->gatewayConnections[$address])) {
                return false;
            }
