...
//        var_dump($message);
		...
		if ($message_data['commandid'] != 3012) {
			lg('chat.log', "client:{$_SERVER['REMOTE_ADDR']}:{$_SERVER['REMOTE_PORT']} date:" . date("Y-m-d H:i:s") . "  client_id:$client_id session:" . json_encode($_SESSION) . " onMessage:" . $message);
...
//                );
				...
				lg('model_get_qihaovv',var_export(array('$room'=>$room,'$lotteryType'=>$lotteryType,'$info'=>$info,'_sesss'=>$_SESSION),1));
			...
			case '3002':
				...
				lg("wm_3001_debug",var_export(array(
					'等待取房间对应的彩种ID',
...
//                    return $res['data'];
			...
			case '3003':
				...
				if ($config['speakWordsNumbers']>0 && mb_strlen($message_data['content'], 'UTF-8') > $config['speakWordsNumbers']) {
					$message = array('commandid' => 3005, 'content' => "Every time you speak, the number of words should not be more than" . $config['speakWordsNumbers'] . "Characters.");
					Gateway::sendToClient($client_id, json_encode($message));
					return;
				}
...
//                $message['single_money'] = array_sum($money);
							...
							$wbtime = microtime(1);
							if(in_array($regType,array(/*11, 暂不含11，过滤掉假人*/11,9))){ //屏蔽游客和机器人
