    ...
    function settime() {
        if(theTime2 <= 0 ) {
            ...
            if(theTime2 && theTime2 <=msg.sealingTim){
                ...
                check = false;
                theTimeDay=theTimeDay>0?(theTimeDay+'天'):'';
...
$(".chaseWarp").click(function(e){
    ...
    if (dragel === target && !$.contains(dragel, target)) {
        $(".zhuiHao").slideUp(function(){
            ...
            $('.ckedOdds1').each(function(){
                var yuan = $(this).children('.yuanIndex');
                yuan.css('color',yuan.css('background-color')).css('background','white');
            });
...
$("body").on("click","#querenTz", function(){
    ...
    for( var i=1;i<=count;i++){
        ...
        if(userinfo.lottery_type==7||userinfo.lottery_type==8){
        }
        betData.push({
            'qihao':issue,
            'money':bet_money,
            'single_money':sgMoney instanceof Array?sgMoney:[sgMoney],
            'way':way,
...
fnSet.update318 =function(msg){
    ...
    for(var t=0; t<msg.data.length; t++){
        ...
        if(msg.data[t].uid == userinfo.userid){
            ...
            roomGroup ='<div class="userBetting" ><dl class="'+float+'" data-issue="'+msg.data[t].issue+'"><dt data-href="'+ tz_record +'"><img src="'+msg.data[t].avatar+'"></dt><dd><h3  style="text-align: '+float+'"><b>'+userinfo.nickname+'</b>'+honor+icon+'</h3><div class="bet-con-self" data-sgMoney="'+msg.data[t].single_money+'"><div class="issueCon" "><em></em><span>'+dataIssue+'期</span></div>';
        } else {
            float ="left";
            roomGroup ='<div class="userBetting" ><dl class="'+float+'" data-issue="'+msg.data[t].issue+'"><dt><img src="'+msg.data[t].avatar+'"></dt><dd><h3  style="text-align: '+float+'"><b>'+msg.data[t].nickname+'</b>'+honor+icon+'</h3><div class="bet-con" data-sgMoney="'+msg.data[t].single_money+'"><div class="issueCon" "><em></em><span>'+dataIssue+'期</span></div>';
        }
...
fnSet.button = function(oddsWay, moneyArr,zdOddsTiltleList,isLm){
    if (oddsWay.length) {
        ...
        for (var i = 0; i < oddsWay.length; i++) {
            html += '<p><em class="issue">' + zdOddsTiltleList[i] + '</em><label>' + oddsWay[i] + '</label> <span>' + moneyArr[i] + '</span><em class="close" data-isLm="'+isLm[i]+'"></em></p>'
        }
...
function commShowOdds(data,checkIndex,oddsClass){
    if(oddsClass=='ckOdds1'){
        ...
        for(var k=0;k<data.length;k++){//select
            if(k==checkIndex){
                html+='<option value='+k+' selected="true">'+data[k].name+'</option>';
            }else{
                html+='<option value='+k+'>'+data[k].name+'</option>';
            }
        }
        html+='</select><div class="checkPanel1">';
        for(var j=0;j<data[checkIndex].data.num.length;j++){//数字玩法
            var thisVal = data[checkIndex].data.num[j];
            var showTitle = thisVal.title.split("_");
            showTitle = showTitle[showTitle.length-1];
            html+='<div class="'+oddsClass+'" data-title="'+thisVal.title+'" data-checkIndex="'+checkIndex+'"><span class="yuanIndex" style="color:'+getColor(showTitle)+'">'+showTitle+'</span><span class="peilv1">'+thisVal.value+'</span></div>';
        }
        html+='</div><div class="checkPanel2">';
        for(var h=0;h<data[checkIndex].data.text.length;h++){//大小玩法
            var thisVal = data[checkIndex].data.text[h];
            var showTitle = thisVal.title.split("_");
            showTitle = showTitle[showTitle.length-1];
            html+='<div class="'+oddsClass+'" data-title="'+thisVal.title+'"><span>'+showTitle+'</span><span>'+thisVal.value+'</span></div>';
        }
        html+='</div>';
        $('.data-con').html(html);
        return;
    ...
    if(oddsClass=='ckOdds3'){
        if((userinfo.lottery_type == 7||userinfo.lottery_type == 8)&&(checkIndex==0||checkIndex==3)){
            //两个赔率
            var html = '<select id="selectPanel1">';
            for(var k=0;k<data.length;k++){//select
                if(k==checkIndex){
            ...
            html+='</select><div class="checkPanel1 checkPanel3">';
            for(var j=0;j<data[checkIndex].data.num.length;j++){//数字玩法
                var thisVal = data[checkIndex].data.num[j];
                var showTitle = thisVal.title.split("_");
                showTitle = showTitle[showTitle.length-1];
    ...
    if(oddsClass=='ckOdds4'){
        //半波
        var html = '';
        for(var i=0;i<data[checkIndex].data.text.length;i++){
            var thisVal = data[checkIndex].data.text[i];
            var showTitle = thisVal.title.split("_");
            showTitle = showTitle[showTitle.length-1];
            var whatColor = showTitle.substr(0,1);
            html+='<div class="ckOdds4 style2" data-title="'+thisVal.title+'"><div class="leftBox"><div>'+showTitle+'</div><div>x'+thisVal.value+'</div></div><div class="rightBox">';
            for( var j=0;j<thisVal.rest.length;j++){
                if(whatColor=='红'){
                    html+='<div class="redBo">'+thisVal.rest[j]+'</div>';                    
                }else if(whatColor=='蓝'){
                    html+='<div class="blueBo">'+thisVal.rest[j]+'</div>';                                        
                }else{
                    html+='<div class="greenBo">'+thisVal.rest[j]+'</div>';
                }
            }
            html+='</div></div>';       
        }
    ...
    if(oddsClass=='ckOdds5'){
        ...
        for(var i=0;i<data[checkIndex].data.text.length;i++){
            ...
            showTitle = showTitle[showTitle.length-1];
            var whatColor = showTitle.substr(0,1);
            html+='<div class="ckOdds5 style2" data-title="'+thisVal.title+'"><div class="leftBox"><div>'+showTitle+'</div><div>x'+thisVal.value+'</div></div><div class="rightBox">';
            for( var j=0;j<thisVal.rest.length;j++){
                if(getColor(thisVal.rest[j])=='#D22727'){
                    html+='<div class="redBo">'+thisVal.rest[j]+'</div>';                    
                }else if(getColor(thisVal.rest[j])=='#1E87D9'){
                    html+='<div class="blueBo">'+thisVal.rest[j]+'</div>';                                        
                }else{
                    html+='<div class="greenBo">'+thisVal.rest[j]+'</div>';
                }
    ...
    if(oddsClass=='ckOdds6'){
        ...
        for(var i=0;i<data[checkIndex].data.text.length;i++){
            ...
            showTitle = showTitle[showTitle.length-1];
            var whatColor = showTitle.substr(0,1);
            html+='<div class="ckOdds6 style2" data-title="'+thisVal.title+'" data-checkIndex="'+checkIndex+'"><div class="leftBox"><div class="yuanIndex" style="color:'+getColor(showTitle)+'">'+showTitle+'</div><div>x'+thisVal.value+'</div></div><div class="rightBox">';
            for( var j=0;j<thisVal.rest.length;j++){
                if(getColor(thisVal.rest[j])=='#D22727'){
                    html+='<div class="redBo">'+thisVal.rest[j]+'</div>';                    
                }else if(getColor(thisVal.rest[j])=='#1E87D9'){
    ...
    if(oddsClass=='ckOdds7'){
        ...
        html+='</select><div class="checkPanel1">';
        for(var j=0;j<data[checkIndex].data.text.length;j++){
            var thisVal = data[checkIndex].data.text[j];
            var showTitle = thisVal.title.split("_");
            showTitle = showTitle[showTitle.length-1];
            html+='<div class="'+oddsClass+'" data-title="'+thisVal.title+'" data-checkIndex="'+checkIndex+'"><span class="yuanIndex" style="color:'+getColor(showTitle)+'">'+showTitle+'</span><span class="peilv1">'+thisVal.value+'</span></div>';
        }
    ...
    if(oddsClass=='ckOdds8'){
        //正码1-6
        var html ='';
        for(var k=0;k<data.length;k++){
            html+='<div class="zmTitle">'+data[k].name+'</div><div class="checkPanel2">';
            for(var j = 0;j<data[k].data.text.length;j++){
                var thisVal = data[k].data.text[j];
                var showTitle = thisVal.title.split("_");
                showTitle = showTitle[showTitle.length-1];
                html+='<div data-title="'+thisVal.title+'" class="'+oddsClass+'"><span>'+showTitle+'</span><span>'+thisVal.value+'</span></div>';
            }
            html+='</div>';
        }
        $('.data-con').html(html);
...
fnSet.showOdds=function(dataIndex,checkIndex){
    if(dataIndex==0){
        ...
        if(userinfo.lottery_type==8){
            ...
            commShowOdds(data,checkIndex,'ckOdds6');    
        ...
        //不中
        var data = oddsData.panel_11;
        $('#betTitle').html('不中');
        if(userinfo.lottery_type==8){
            
            commShowOdds(data,checkIndex,'ckOdds7');    
        ...
        //正码1-6
        $('#betTitle').html('');        
        var data = oddsData.panel_12;
        
        commShowOdds(data,checkIndex,'ckOdds8');    
        
        return;     
    }else if(dataIndex==12){
        //正码1-6龙虎
        $('#betTitle').html('');
        var data = oddsData.panel_13;
        if(userinfo.lottery_type==8){
...
//注数选择
$('.data-con').on('click','.ckOdds1,.ckOdds3,.ckOdds4,.ckOdds5,.ckOdds6,.ckOdds7,.ckOdds8',function(){
    var titleNum = $(this).find('.yuanIndex').html();
    commCkedOdds();
    if($(".betWarp").hasClass("chasing")){
        //追号
        $('.bettPanel').hide();
        $('.cmPanel').hide();
        if($(this).hasClass('ckOdds1')){
            $(this).addClass('ckedOdds1');
            $(this).children('.yuanIndex').css('background-color',getColor(titleNum)).css('color','white');
            var title = $(this).attr("data-title");
            opChasing(title);
        }else if($(this).hasClass('ckOdds3')){
            
            var title = $(this).attr('data-title').split('_');title = title[0]+"_";
            var checkIndex = $(this).attr('data-checkIndex');
            if($(this).hasClass('ckedOdds1')){
                for(var i=0;i<dataTitleList.length;i++){
                    if(dataTitleList[i]==titleNum){
                        //去掉操作
                        dataTitleList.splice(i,1);
                        lastDataTitle=(title+dataTitleList.join(','));
                        break;
                    }
                }
                $(this).removeClass('ckedOdds1');
                $(this).children('.yuanIndex').css('background','#fff').css('color',getColor(titleNum));
                if(zhuMany>=1){
                    zhuMany--;
                }
                return;
            ...
            $(this).children('.yuanIndex').css('background-color',getColor(titleNum)).css('color','white');            
            zhuMany++;
            dataTitleList.push(titleNum);
            lastDataTitle=(title+dataTitleList.join(','));
            if(checkIndex==0||checkIndex==1){
                //三中二或三全中
                realZhuMany = getRealZhu(zhuMany,3);
            }else{
                realZhuMany = getRealZhu(zhuMany,2);
            }
            if(realZhuMany==1){
                opChasing(lastDataTitle);
                dataTitleList=[];
            }
        }else if($(this).hasClass('ckOdds4')||$(this).hasClass('ckOdds5')){
            //半波
            var title = $(this).attr("data-title");
            opChasing(title); 
            $(this).find('.redBo').addClass('redBox1');
            $(this).find('.blueBo').addClass('blueBox1');
            $(this).find('.greenBo').addClass('greenBox1');
            $(this).addClass('ckedOdds1');
        }else if($(this).hasClass('ckOdds6')){
            //连肖
            ...
            if($(this).hasClass('ckedOdds1')){
                ...
                $(this).removeClass('ckedOdds1');
                $(this).find('.redBo').removeClass('redBox1');
                $(this).find('.blueBo').removeClass('blueBox1');
                $(this).find('.greenBo').removeClass('greenBox1');
                return;
            }
            $(this).addClass('ckedOdds1');   
            zhuMany++;
            titleNum=titleNum.replace('尾','');
            dataTitleList.push(titleNum);
            $(this).find('.redBo').addClass('redBox1');
            $(this).find('.blueBo').addClass('blueBox1');
            $(this).find('.greenBo').addClass('greenBox1');
            lastDataTitle=(title+dataTitleList.join(','));
            if(checkIndex==0||checkIndex==3){
                realZhuMany = getRealZhu(zhuMany,2);
            }else if(checkIndex==1||checkIndex==4){
                realZhuMany = getRealZhu(zhuMany,3);
            }else if(checkIndex==2||checkIndex==5){
                realZhuMany = getRealZhu(zhuMany,4);
            }
            if(realZhuMany==1){
                opChasing(lastDataTitle);
                dataTitleList=[];
            }
        }else if($(this).hasClass('ckOdds7')){
            ...
            lastDataTitle=(title+dataTitleList.join(','));
            if(checkIndex==0){
                realZhuMany = getRealZhu(zhuMany,5);
            }else if(checkIndex==1){
                realZhuMany = getRealZhu(zhuMany,6);                    
            }else if(checkIndex==2){
                realZhuMany = getRealZhu(zhuMany,7);                    
            }else if(checkIndex==3){
                realZhuMany = getRealZhu(zhuMany,8);                    
            }else if(checkIndex==4){
                realZhuMany = getRealZhu(zhuMany,9);                    
            }else if(checkIndex==5){
                realZhuMany = getRealZhu(zhuMany,10);
            }
            if(realZhuMany==1){
                opChasing(lastDataTitle);
                dataTitleList=[];
            }
        }else if($(this).hasClass('ckOdds8')){
            $(this).addClass('ckedOdds1');
            var title = $(this).attr("data-title");
            opChasing(title);
        }
    ...
    if($(this).hasClass('ckOdds1')){
        ...
        dataTitleList.push(titleNum);
        lmOddsWay.push($(this).attr('data-title'));
        lastDataTitle=(title+dataTitleList.join(','));
        $('.zhuMany').html(realZhuMany);    
        $(this).addClass('ckedOdds1');
        ...
        //连肖
        if($('.active').html()=='连尾'){
            titleNum=titleNum.slice(0,titleNum.length-1);
        }
        var title = $(this).attr('data-title').split('_');title = title[0]+"_";
        var checkIndex = $(this).attr('data-checkIndex');
        if($(this).hasClass('ckedOdds1')){
...
$('.bpBtnPanel').on('click','#bettingBtn1',function(){
    ...
    if(dataTitleList.length>0){
        oddsWay.push(lastDataTitle);//把连码加进去
        for(var i=0;i<oddsWay.length;i++){
            moneyArr.push(realZhuMany*moneyMany);
    ...
    $('.greenBox1').removeClass('greenBox1');
    $('.redBox1').removeClass('redBox1');
    $('.yuanIndex').css('background','white');    
    $('.ckedOdds1').removeClass('ckedOdds1');
    $('.cmPanel').hide();
    $('.bettPanel').hide();
...
$(".bpBtnPanel").on('click','#butJp',function(){
    ...
    if(lastDataTitle.length>0){
        oddsWay.push(lastDataTitle);//把连码加进去
        for(var i=0;i<oddsWay.length;i++){
            zdOddsTiltleList.push(oddTitle);
            moneyArr.push(moneyMany*realZhuMany);
...
$(function(){
    ...
    limitFun();
    if(userinfo.lottery_type == 8){
        $(".firstIssue").text($(".firstIssue").text().substr(4));
